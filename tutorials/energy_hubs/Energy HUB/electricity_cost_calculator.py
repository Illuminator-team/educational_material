...
def calculate_electricity_cost(
    ...
    if len(result) == 0:
        rint("⚠ WARNING: No timestamps matched!")
        return None
    
    # Calculate baseline cost (without renewables)
    result['Grid_Energy_MWh'] = result['Energy_MWh']
    result['Cost_EUR'] = result['Grid_Energy_MWh'] * result['Price_EUR_MWh']
    ...
    if new_settings and any([pv_output_file, wind_output_file]):
        ...
        if renewable_available:
            ...
            # Calculate grid cost with renewables
...
def plot_electricity_cost(result_df, title='Electricity Cost Over Time'):
    """
    Plot electricity costs per timestep only.
    ...
    print("\n" + "="*70)
    print("PLOTTING ELECTRICITY COSTS")
    print("="*70)
    
    fig, ax = plt.subplots(1, 1, figsize=(15, 6))
    
    # Plot cost per timestep
    ax.plot(result_df.index, result_df['Cost_EUR'], 
            linewidth=1.5, color='#2ca02c', label='Cost per timestep')
...
if __name__ == "__main__":
    ...
    results_renewable = calculate_electricity_cost(
        ...
        price_date_column='MTU (CET/CEST)',
        price_column='Price (EUR/MWh)',
        pv_output_file='outputs\pv_generation.csv',  # Optional
        pv_column='PV_power_kW',              # Optional
        wind_output_file='outputs\wind_generation.csv',  # Optional
